...
interface BuildOrderListProps { buildOrder: BuildOrder; }
    ...
    <div className="space-y-5">
      ...
      <div className="bg-white/[0.05] border border-white/[0.09] rounded-2xl p-5">
        <div className="flex items-start justify-between gap-4">
          ...
          <span className="flex-shrink-0 rounded-full border border-white/10 px-3 py-1 text-xs text-white/40">
            {DIFFICULTY_LABELS[buildOrder.difficulty]}
          </span>
        </div>
        <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
          <div>
            <p className="text-[11px] font-medium tracking-widest uppercase text-white/40 mb-2">Strengths</p>
          ...
          <div>
            <p className="text-[11px] font-medium tracking-widest uppercase text-white/40 mb-2">Weaknesses</p>
            <ul className="space-y-1">
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          <p className="text-[11px] font-medium tracking-widest uppercase text-white/40">
            Build Steps <span className="normal-case text-white/20 font-normal ml-1">({steps.length})</span>
          </p>
          <button onClick={() => setShowOptional(!showOptional)} className="text-[11px] text-white/30 hover:text-white/60 transition-colors">
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        <div className="bg-white/[0.05] border border-white/[0.09] rounded-2xl p-4">
          <p className="text-[11px] font-medium tracking-widest uppercase text-white/40 mb-2">Late Game</p>
          <p className="text-xs text-white/50 leading-relaxed">{buildOrder.lateGame}</p>
        </div>
        <div className="bg-white/[0.05] border border-white/[0.09] rounded-2xl p-4">
          <p className="text-[11px] font-medium tracking-widest uppercase text-white/40 mb-2">General Tips</p>
          <ul className="space-y-1.5">
            {buildOrder.generalTips.map((tip) => (
              <li key={tip} className="flex items-start gap-1.5 text-xs text-white/50">
                <span className="text-amber-500/50 mt-0.5 flex-shrink-0">→</span>{tip}
              </li>
            ))}
          </ul>
        </div>
